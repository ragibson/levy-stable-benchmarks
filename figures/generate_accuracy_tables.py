...
if __name__ == "__main__":
    ...
    for density_function, use_absolute_tolerances in zip(["CDF", "CDF", "PDF", "PDF"], [True, False, True, False]):
        ...
        for i, table in enumerate(tables):
            ...
            print("<table>\n"
                  f"  <tr><td></td><td colspan=\"4\">{tolerance_label}</td><td>Average Time Per Call</td></tr>\n"
                  f"  <tr><td>Method</td>{tolerances_header}<td></td></tr>")
            for method, name in zip(methods, method_names):
                ...
                print("    " + "".join(f"<td>{np.floor(1000 * accuracy) / 10:.1f}%</td>" for accuracy in accuracies))
                print("    <td></td>\n"
                      "  </tr>")
            print(r"</table>")
